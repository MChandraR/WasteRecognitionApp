...
class UserRepository : ApiService() {
    fun getUserRepository(request : suspend (userService : UserService)->Unit){
        retrofit?.create(UserService::class.java)?.let {
    ...
    fun login(loginBody : LoginModelRequestAPI, onSuccess: (responseBody : ResponseBody<LoginResponseModelAPI>)->Unit, onFailed: (message:String)->Unit) {
        this.getUserRepository{ it ->
            var loginResponse = it.login(credential = loginBody)
